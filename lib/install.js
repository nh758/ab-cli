...
function cloneRepo(done) {
   ...
   shell.exec(
      `git clone https://github.com/nh758/ab_runtime.git ${Options.name}`
   );
...
function compileUI(done) {
   ...
   shell.exec("node node_modules/webpack-cli/bin/cli.js");
   shell.popd("-q");
   console.log("\ncompile ABDesigner UI");
   shell.pushd("-q", path.join(process.cwd(), "developer", "plugins", "ABDesigner"))
   shell.exec("node node_modules/webpack-cli/bin/cli.js");
   shell.popd("-q");
   done();
}
